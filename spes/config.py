...
@dataclass
class SPESConfig(BaseConfig):
    """
    Config for Sparse Expert Sync Training
    
    """
    
    server_addr: Optional[str] = None
    """
    Address of parameter server, format: ip_addr: port
    """

    num_peers: int = 5
    """
    Number of decentralized training nodes
    """

    peer_id: int = 0
    """
    identification of current node
    """

    num_train_experts_per_node: int = 1
    """
    Number of trainable experts per node
    """

    quant_non_local_trainable_experts: bool = False
    """
    Whether to quant non local experts
    """

    sync_steps: int = 100
    """
    Number of local steps between each expert parameter synchronization across nodes.
    """

    using_decayed_aux_loss: bool = False
    """
    Whether to use decayed load balance loss
    """

    decayed_factor_update_steps: int = 500
    """
    Used to adjust the frequency of decay factor update 
    """
...
class ModelConfig(BaseConfig):
    ...
    """
    attention_layer_norm_over_head: bool = False
    """
    Apply layer norm to query and keys within the attention mechanism.
    But over head dim. (To upsyscling training from qwen3)
    """
    ...
    using_custom_moe_impl: Optional[bool] = False
    """
    Whether to using custom moe impl
    """
...
class TrainConfig(BaseConfig):
    ...
    using_spes: bool = False
    """
    Whether using SPES (mem-efficient training for MoE LLM)
    """
    using_dilico: bool = False
    """
    Whether to train full model during local training
    """

    spes_config: SPESConfig = field(default_factory=SPESConfig)
    """
    Config for spes training
    """
