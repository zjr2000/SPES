...
from .decayed_load_balance_loss import set_current_trainable_expert_loss_factor
...
from .spes import (
    federated_pb2,
    federated_pb2_grpc,
    upload_weights_in_chunks,
    download_weights_in_chunks
)
...
class Trainer:
    ...
    def __post_init__(self):
        ...
        if self.cfg.using_spes:
            channel = grpc.insecure_channel(
                self.cfg.spes_config.server_addr,
                options=[
            ...
            self.spes_stub = federated_pb2_grpc.FederatedServerStub(channel)
            ...
            with grad_sync_context():
                ...
                if self.model.config.block_type == BlockType.moe:
                    if self.model.config.moe_zloss_weight:
                        lb_loss = megablocks.layers.moe.batched_load_balancing_loss(self.moe_args)
                        lb_loss = lb_loss / len(micro_batches)
                        moe_z_loss = batched_router_zloss(self.moe_args).sum()
                        moe_z_loss = moe_z_loss / len(micro_batches)
                        clear_router_zloss()
                    elif self.model.config.moe_loss_weight:
                        lb_loss = megablocks.layers.moe.batched_load_balancing_loss(self.moe_args) / len(micro_batches)
        ...
        with torch_profiler as p:
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in self.train_loader:
                    ...
                    # [SPES] Sync experts' weights with server
                    self.spes_sync_if_needed()

                    if self.cfg.using_spes and self.cfg.spes_config.using_decayed_aux_loss and self.global_step % self.cfg.spes_config.decayed_factor_update_steps == 0:
                        set_current_trainable_expert_loss_factor(self.global_step, self.max_steps)
    ...
    def spes_sync_if_needed(self):
        with torch.no_grad():
            if not self.cfg.using_spes:
                return

            if self.global_step == 0 or (self.global_step % self.cfg.spes_config.sync_steps != 0):
                    return

            syncronized_weights = None
            payload = self._serialize_state_dict()
            if get_global_rank() == 0:
                # Serialize
                log.info(f"[Peer {self.cfg.spes_config.peer_id}] Upload weights to server at step {self.global_step}")
                upload_weights_in_chunks(self.spes_stub, self.cfg.spes_config.peer_id, self.global_step, payload)
                
                while True:
                    syncronized_weights = download_weights_in_chunks(self.spes_stub, self.cfg.spes_config.peer_id, self.global_step)
                    if syncronized_weights:
                        break
                    else:
                        log.info(f"[{self.cfg.spes_config.peer_id}] Aggregated weights not ready, sleeping 10s...")
                        time.sleep(5)
                
            self._load_state_dict_from_bytes(syncronized_weights)
            barrier()
            torch.cuda.empty_cache()

    def _fill_with_nan(self) -> None:
        """
        把模型里所有参数用 NaN 填充；用于之后的完整性校验。
        """
        with torch.no_grad():
            for p in self.dist_model.parameters():
                p.data.fill_(math.nan)

    def _assert_no_nan(self, tag: str) -> None:
        """
        如果任一参数仍有 NaN，直接抛错。
        """
        for name, p in self.dist_model.named_parameters():
            if torch.isnan(p).any():
                raise RuntimeError(f"{tag}: parameter '{name}' still contains NaN after loading checkpoint")

    def _load_state_dict_from_bytes(self, payload: bytes) -> None:
        """
        将 server 返回的 bytes 加载进当前模型；自动区分 FSDP / DDP。
        """
        if isinstance(self.dist_model, FSDP):
            ...
            with FSDP.state_dict_type(
                ...
                full_state_dict = torch.load(buffer, map_location="cpu")
                self._fill_with_nan()
                self.dist_model.load_state_dict(full_state_dict)
                self._assert_no_nan(tag=f"rank{get_global_rank()}-FSDP")

        elif isinstance(self.dist_model, DDP):
            if get_global_rank() == 0:
                buffer = io.BytesIO(payload)
                full_state_dict = torch.load(buffer, map_location="cpu")
                self.dist_model.module.load_state_dict(full_state_dict)
            # 同步所有参数
            for param in self.dist_model.module.parameters():
                dist.broadcast(param.data, src=0)
        else:
            self.dist_model.load_state_dict(state_dict)

    def _serialize_state_dict(self) -> bytes:
        """
        根据分布式策略返回可直接 over-the-wire 传输的权重 bytes。
        FSDP 需要先切换到 FULL_STATE_DICT，再交给 torch.save。
        """
        if isinstance(self.dist_model, FSDP):
            ...
            with FSDP.state_dict_type(
                self.dist_model, 
                state_dict_type=StateDictType.FULL_STATE_DICT, 
                state_dict_config=full_cfg
            ):
                state_dict = self.dist_model.state_dict()
        elif isinstance(self.dist_model, DDP):
            # DDP wrapper里真正的模型在 module 属性
            state_dict = self.dist_model.module.state_dict()
        else:  # 单卡
            state_dict = self.dist_model.state_dict()

        state_dict = {k: v for k, v in state_dict.items() if k in self.trainable_module_keys}
