...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.attention_layer_norm_over_head = config.attention_layer_norm_over_head
        if config.attention_layer_norm:
            assert config.effective_n_kv_heads is not None
            if config.attention_layer_norm_over_head:
    ...
    def attention(
        ...
        if self.q_norm is not None and self.k_norm is not None and not self.attention_layer_norm_over_head:
        ...
        if self.q_norm is not None and self.k_norm is not None and self.attention_layer_norm_over_head:
            q = self.q_norm(q).to(dtype=dtype)
            k = self.k_norm(k).to(dtype=dtype)
...
class OLMoEBlock(OLMoBlock):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self.ffn.experts.mlp, "expert_w1"):
            for expert_idx in range(len(self.ffn.experts.mlp.expert_w1)):
                init_normal(self.ffn.experts.mlp.expert_w1[expert_idx], std=in_std, init_cutoff_factor=cutoff_factor)
                init_normal(self.ffn.experts.mlp.expert_v1[expert_idx], std=in_std, init_cutoff_factor=cutoff_factor)
                init_normal(self.ffn.experts.mlp.expert_w2[expert_idx], std=in_std, init_cutoff_factor=cutoff_factor)
        else:
