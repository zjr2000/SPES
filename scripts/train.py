...
from spes.data import build_train_dataloader
from spes.eval import build_evaluators
from spes.exceptions import OLMoCliError, OLMoConfigurationError
from spes.model import OLMo
from spes.optim import BoltOnWarmupScheduler, build_optimizer, build_scheduler
from spes.torch_util import (
    SingleAccelerator,
    barrier,
    get_default_device,
    get_global_rank,
...
from spes.train import Trainer
from spes.util import (
...
def main(cfg: TrainConfig) -> None:
    ...
    if (cfg.using_spes or cfg.model.using_custom_moe_impl) and cfg.model.block_type == "moe" and cfg.model.moe_mlp_impl == "sparse":
        if cfg.spes_config.using_decayed_aux_loss:
            from spes.decayed_load_balance_loss import batched_load_balancing_loss as decayed_batched_load_balance_loss
            from spes.decayed_load_balance_loss import set_trainable_expert_indices
            import megablocks.layers.moe
            megablocks.layers.moe.batched_load_balancing_loss = decayed_batched_load_balance_loss
            group_idx = cfg.spes_config.peer_id
            start_idx = group_idx * cfg.spes_config.num_train_experts_per_node
            end_idx = min(start_idx + cfg.spes_config.num_train_experts_per_node, cfg.model.moe_num_experts)
            trainable_expert_indices = set(range(start_idx, end_idx))
            set_trainable_expert_indices(trainable_expert_indices)



        from spes.custom_sparse_glu_impl import CustomSparseGLU
        from megablocks.layers import dmlp_registry
        # Monkey Patching, using custom hacking impl for frozn some experts
        dmlp_registry._REGISTRY['glu']['sparse'] = CustomSparseGLU

    olmo_model = OLMo(cfg.model)

    if cfg.using_dilico:
        log.info(f"using_dilico {cfg.using_dilico}, using_spes: {cfg.using_spes}")

    if cfg.using_spes and not cfg.using_dilico:
        peer_id = cfg.spes_config.peer_id
        num_peers = cfg.spes_config.num_peers
        num_train_experts_per_node = cfg.spes_config.num_train_experts_per_node
        quant_non_local_trainable_experts = cfg.spes_config.quant_non_local_trainable_experts

        group_idx = peer_id
        start_idx = group_idx * num_train_experts_per_node
        end_idx = min(start_idx + num_train_experts_per_node, cfg.model.moe_num_experts)
        trainable_expert_indices = set(range(start_idx, end_idx))
        # frozen non local trainable experts
        for block in olmo_model.transformer.blocks:
            for expert_idx in range(cfg.model.moe_num_experts):
                if expert_idx in trainable_expert_indices:
                    continue

                block.ffn.experts.mlp.expert_w1[expert_idx].requires_grad = False
                block.ffn.experts.mlp.expert_v1[expert_idx].requires_grad = False
                block.ffn.experts.mlp.expert_w2[expert_idx].requires_grad = False

    trainable_module_keys = {k for k, v in olmo_model.named_parameters() if v.requires_grad}

    log.info(f"Total number of parameters: {olmo_model.num_params():,d}")
    log.info(f"Num trainable parameters: {olmo_model.num_trainable_params():,d}")
    ...
    with Trainer(
        ...
        trainable_module_keys=trainable_module_keys
    ) as trainer:
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        dist.init_process_group(
            backend="nccl", timeout=timedelta(minutes=240), device_id=torch.device(device_as_string)
        )
    elif torch.backends.mps.is_available():
