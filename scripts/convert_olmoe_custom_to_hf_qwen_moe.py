...
import torch
model = Qwen3MoeForCausalLM.from_pretrained("../transformers/olmoe", torch_dtype=torch.bfloat16).cuda()
model = Qwen3MoeForCausalLM.from_pretrained("../transformers/olmoe").cuda()
...
def write_model(model_path, input_base_path, tokenizer_path=None, safe_serialization=True, fix_eos_token_id=True):
    ...
    for layer_i in range(n_layers):
        ...
        dim_per_expert = loaded[f"transformer.blocks.{layer_i}.ffn.experts.mlp.expert_w1.0"].shape[0]
        for expert_i in range(num_experts):
            state_dict[f"model.layers.{layer_i}.mlp.experts.{expert_i}.gate_proj.weight"] = loaded[
                f"transformer.blocks.{layer_i}.ffn.experts.mlp.expert_w1.{expert_i}"
            ]
            state_dict[f"model.layers.{layer_i}.mlp.experts.{expert_i}.up_proj.weight"] = loaded[
                f"transformer.blocks.{layer_i}.ffn.experts.mlp.expert_v1.{expert_i}"
            ]
            state_dict[f"model.layers.{layer_i}.mlp.experts.{expert_i}.down_proj.weight"] = loaded[
                f"transformer.blocks.{layer_i}.ffn.experts.mlp.expert_w2.{expert_i}"
            ].T.contiguous()
    ...
    config = Qwen3MoeConfig(
        ...
        num_experts_per_tok=olmoe_config["moe_top_k"],
        norm_topk_prob=True,
        moe_intermediate_size=dim_per_expert,
    ...
    model = Qwen3MoeForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.bfloat16)
...
def _write_tokenizer(
    output_path: Path, config: Qwen3MoeConfig, input_tokenizer_path: Path, fix_eos_token_id: bool = True
) -> None:
